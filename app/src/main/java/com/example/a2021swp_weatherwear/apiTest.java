...
public class apiTest extends Thread
{
    ...
    public void func(String year, String month, String day) throws IOException, JSONException
    {
        ...
        String s5 = endPoint + "/getVilageFcst?serviceKey=" + serviceKey
                + "&pageNo=" + pageNo
                + "&numOfRows=" + numOfRows
                + "+&dataType=JSON"
                + "&base_date=" + baseDate
        ...
        else if(conn2.getResponseCode() >= 200 && conn2.getResponseCode() <= 300) {
            bufferedReader = new BufferedReader(new InputStreamReader(conn2.getInputStream()));
        }
        else if(conn3.getResponseCode() >= 200 && conn3.getResponseCode() <= 300) {
            bufferedReader = new BufferedReader(new InputStreamReader(conn3.getInputStream()));
        }
        else if(conn4.getResponseCode() >= 200 && conn4.getResponseCode() <= 300) {
            bufferedReader = new BufferedReader(new InputStreamReader(conn4.getInputStream()));
        }
        else if(conn5.getResponseCode() >= 200 && conn5.getResponseCode() <= 300) {
            bufferedReader = new BufferedReader(new InputStreamReader(conn5.getInputStream()));
        }
        else {
